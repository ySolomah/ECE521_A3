...
with np.load("notMNIST.npz") as data:
    ...
    for hiddenCount, colour in zip(num_neurons, ['mo', 'bo', 'ro', 'go', 'yo']):
        ...
        dropoutMap = tf.nn.dropout(featureMap, keep_prob)
        outputLayer, regularizeW2, W2, B2 = fcLayer(dropoutMap, 10, spatialSize=hiddenCount, lamb=lam)        
        ...
        temp5_test = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits= tf.matmul(featureMap, W2) + B2, labels=y))
        softmax_acc_test = tf.argmax(tf.nn.softmax(tf.matmul(featureMap, W2) + B2), dimension=1)
                ...
                for miniBatchData, miniBatchTarget in zip(trainDataReshaped, trainTargetReshaped):
                    ...
                    if(new_epoch):
                        if(save and saver == 3 and epoch > 0.99 * int(num_iter / trainDataReshaped.shape[0])):
                            tf.train.Saver().save(sess, '/Users/yassirsolomah/ECE521_A3/ECE521_A3/99save1_3')
                            saver = saver + 1
                        if(save and saver == 2 and epoch > 0.74 * int(num_iter / trainDataReshaped.shape[0])):
                            tf.train.Saver().save(sess, '/Users/yassirsolomah/ECE521_A3/ECE521_A3/74save1_3')
                            saver = saver + 1
                        if(save and saver == 1 and epoch > 0.49 * int(num_iter / trainDataReshaped.shape[0])):
                            tf.train.Saver().save(sess, '/Users/yassirsolomah/ECE521_A3/ECE521_A3/49save1_3')
                            saver = saver + 1
                        if(save and saver == 0 and epoch > 0.24 * int(num_iter / trainDataReshaped.shape[0])):
                            tf.train.Saver().save(sess, '/Users/yassirsolomah/ECE521_A3/ECE521_A3/24save1_3')
                            saver = saver + 1
                        new_epoch = False
                        if(epoch % 100 == 0):
                        ...
                        loss_array.append(accuracy*100)
                       	...
                       	for typeAcc, guessIn, guessOut, truthCopy, accuracy_array, loss_array in \
                       		...
                       		guesses = sess.run(softmax_acc_test, feed_dict={X: guessIn, y: guessOut})
	                        accuracy = 1 - (((np.absolute(guesses - truthCopy)).clip(0, 1).sum())/guesses.shape[0])
	                       	entropy_loss = sess.run(temp5_test, feed_dict={X: guessIn, y: guessOut})/guesses.shape[0]
